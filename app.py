...
def get_profile_path(uid):
    return os.path.join(PROFILES_DIR, f"{uid}_profile.json")


def get_avatar_path(uid):
    return os.path.join(PROFILES_DIR, f"{uid}_avatar.jpg")
...
def status():
    ...
    return jsonify(
        {
            "retrain_date": info.get("date") if info else None,
...
def start():
    ...
    if bot.memory.is_returning():
        ...
        greeting = (
            f"{tg}, {name}. I'm glad you came back. How are you feeling today?"
        ...
        bot.memory.start_session()
        greeting = (
            f"{tg}, {name}! I'm MindBridge — your private emotional support companion. "
    ...
    profile = client_memory.get(
        "profile", {"name": name, "joined": str(datetime.date.today()), "bio": ""}
    )
    return jsonify(
        {
            "greeting": greeting,
...
def upload_memory():
    ...
    bot = active_bots[uid]
    prompt = (
        f"User {name} just shared a photo memory with the caption: '{caption}'. "
    ...
    try:
        ...
        response = (
            f"Thank you for sharing this memory with me, {name}. "
...
def get_stats():
    ...
    mid = now.replace(hour=0, minute=0, second=0, microsecond=0) + datetime.timedelta(
        days=1
    )
...
def get_history():
    ...
    available = sorted(
        set(e.get("date", "") for e in bot.memory.data.get("emotion_timeline", []))
    )
    ...
    for i, sess in enumerate(sessions):
        ...
        try:
            t_start = datetime.datetime.fromisoformat(
                sess[0].get("timestamp", "")
            ).strftime("%I:%M %p")
            t_end = datetime.datetime.fromisoformat(
                sess[-1].get("timestamp", "")
            ).strftime("%I:%M %p")
        except Exception:
            t_start = t_end = "—"
...
def personal_model():
    ...
    return jsonify(
        {
            "observations": cal.total_observations,
            "patterns": pairs[:10],
            "hidden_emotions": hints,
            "knowledge_graph": kg,
...
def analyze_graph():
    ...
    for d in dates:
        s = sum(
            cnt if em in POSITIVE_EMOTIONS else -cnt if em in NEGATIVE_EMOTIONS else 0
...
def motivation():
    ...
    MOTIVATIONS = [
        (
            "The only way out is through. You are stronger than you think, and this moment — as heavy as it feels — is not the end of your story.",
            None,
        ),
        (
            "Every morning you wake up and try again is an act of incredible courage. Do not underestimate that.",
            None,
        ),
        (
        ...
        (
            "The version of you that got through every hard day so far is the same one getting through this one.",
            None,
        ),
        (
            "Progress is not always visible. Sometimes healing, growing, and becoming happen quietly inside you.",
            None,
        ),
        (
            "You are not behind. You are not failing. You are on your own timeline and that is perfectly valid.",
            None,
        ),
        (
            "It's okay to rest. It's okay to not be okay. And it's okay to need time. None of that makes you weak.",
            None,
        ),
        (
            "The people who seem the strongest have simply decided that giving up is not an option they're willing to choose.",
            None,
        ),
        (
            "Your feelings are valid. Your struggles are real. And your capacity to get through them is greater than you know.",
            None,
        ),
        (
            "One day at a time. Sometimes one hour. Sometimes one breath. That's enough.",
            None,
        ),
        (
            "The fact that you're still here, still trying — that is not nothing. That is everything.",
            None,
        ),
        (
            "Growth is uncomfortable because you are expanding into something larger than you've ever been before.",
            None,
        ),
        (
            "You have survived every hard day you've faced so far. Your track record is 100%.",
            None,
        ),
        (
        ...
        (
            "You are allowed to be both a work in progress and worthy of love and belonging right now.",
            "Brené Brown (adapted)",
        ),
        (
            "Difficult roads often lead to beautiful destinations. Trust the process even when the path is unclear.",
            None,
        ),
        (
            "Your sensitivity is not a weakness. It means you feel deeply, care deeply, and live deeply.",
            None,
        ),
        (
        ...
        (
            "Healing is not linear. Some days you'll feel fine and some days you won't. Both are part of the journey.",
            None,
        ),
        (
            "You are not the sum of your worst days. You are also every small act of courage, every time you kept going.",
            None,
        ),
        ...
        (
            "Stars can't shine without darkness. Your darkest moment is preparing your brightest chapter.",
            None,
        ),
        (
            "What you are going through is hard. What you are becoming because of it is extraordinary.",
            None,
        ),
        (
            "You don't have to be perfect to be enough. You already are enough, exactly as you are right now.",
            None,
        ),
        (
...
def receive_gradient():
    ...
    try:
        ...
        print(
            f"[FL] Gradient received from {request.remote_addr} "
            f"→ {path} ({n} samples)"
        )
...
if __name__ == "__main__":
    ...
    print(
        f"  Microphone    : {'ENABLED (HTTPS active)' if use_https else 'localhost only (HTTP)'}"
    )
    if info:
        print(
            f"  Last retrain  : {info.get('date')} — acc {info.get('avg_accuracy',0):.2%}"
        )
    print(f"{'='*58}")
    if use_https:
